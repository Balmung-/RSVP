...
// Rebuild an internal-shaped `messages` array from the stored
// ChatMessage rows of a session. The storage shape is flat (one row
// per "logical" message) but the runtime contract wants:
...
//
// Tool-use IDs inside a single turn must match between the tool_use
// block and its corresponding tool_result. The IDs from the live
// stream are sealed at end-of-turn — when we REPLAY a past turn, we
// regenerate a stable id from the stored tool row's CUID:
// `toolu_<rowId>`. Providers don't care what the id is as long as
// it's consistent within the request, so this is safe.
//
// This module speaks ONLY internal runtime types (see ./runtime/
// types.ts). The provider-specific translation happens inside each
// AIRuntime implementation — this module stays provider-agnostic so
// OpenRouter / any future backend gets the same replay behavior.
...
export function rebuildMessages(rows: ChatMessage[]): InternalMessage[] {
  ...
  while (i < rows.length) {
    ...
    if (row.role === "assistant") {
      const blocks: Array<InternalTextBlock | InternalToolUseBlock> = [];
      ...
      while (j < rows.length && rows[j].role === "tool") {
        ...
        // blob, but the provider has no way to distinguish "the tool
        ...
        const resultParam: InternalToolResultBlock = {
          type: "tool_result",
          tool_use_id: id,
      ...
      if (blocks.length === 0) {
        ...
        // tool calls. Emit a single-space text block so providers
        // don't reject the turn — content can't be empty.
...
// Given an in-flight assistant content array (the blocks accumulated
// from the CURRENT streaming turn), convert them into an assistant
// InternalMessage suitable for appending to messages[] on the next
// loop iteration. Text blocks pass through; tool_use blocks keep
// their live provider IDs (which match the tool_use_ids we just
// handed to tool_result). This is the one place in the flow where
// we use the live IDs rather than synthesized ones.
export function assistantTurnFromBlocks(
  blocks: InternalAssistantContent,
): InternalMessage {
  const params: InternalAssistantContent = blocks.map((b) => {
    if (b.type === "text") {
