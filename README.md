...
- **Production fail-fast at boot.** Next.js [`instrumentation.ts`](src/instrumentation.ts) calls the runtime guard once per server start. When `NODE_ENV=production` and the AI backend is misconfigured, the guard throws from `register()` and the server fails to start — preferable to booting into a state where every `/api/chat` request 503s. In dev and test, the guard warns and continues so local work without an AI key still boots.
