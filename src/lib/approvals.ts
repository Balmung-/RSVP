...
import { sendCampaign, type SendCampaignChannel } from "./campaigns";
...
export async function requestApproval(params: {
  ...
  if (campaign) {
    await notifyTenantAdmins(
      ...
      `${requester?.fullName ?? requester?.email ?? "An editor"} is asking to send ${params.recipientCount.toLocaleString()} ${channelLabel(params.channel)} for "${campaign.name ?? "a campaign"}".\n\nThe send is paused until a workspace admin approves.`,
      "/approvals",
    );
  }
...
export async function decideApproval(
  ...
  if (decision === "approved") {
    await sendCampaign(row.campaignId, {
      channel: row.channel as SendCampaignChannel,
      onlyUnsent: true,
    });
  }
...
function channelLabel(channel: SendCampaignChannel): string {
  switch (channel) {
    case "email":
      return "emails";
    case "sms":
    ...
    case "all":
      return "messages across all available channels";
    case "both":
    default:
      return "messages";
  }
