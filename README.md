...
- **Production fail-fast at boot.** Next.js [`instrumentation.ts`](src/instrumentation.ts) calls the runtime guard once per server start. Requires `experimental.instrumentationHook: true` in [`next.config.js`](next.config.js) on Next 14 — without the flag the hook is silently skipped. When `NODE_ENV=production` and the AI backend is misconfigured, the guard throws AND the wrapper calls `process.exit(1)` — Next 14 catches throws from `register()` and would otherwise keep serving 500s to every request, so the hard exit is what actually crashes the process and lets the platform surface the failed deploy. In dev and test, the guard warns and continues so local work without an AI key still boots.
